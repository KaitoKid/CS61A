...
def cumulative_sum(t):
    ...
    if t.is_leaf():
        ...
        sum_b = [cumulative_sum(b) for b in branches(t)]
        return Tree((sum(b.label for b in sum_b) + t.label), sum_b)
...
def reverse(link):
    ...
    if link == Link.empty:
        return link
    tlink = Link(link.first)
    link = link.rest
    while link != Link.empty:
        tlink = Link(link.first, tlink)
        link = link.rest
    return tlink
...
def is_valid_bst(bst):
    ...
    True
    """
    if bst.left.label < bst.label and bst.right.label > bst.label:
        if bst_max(bst.left) < bst.label and bst_min(bst.right) > bst.label:
            return True
    else:
        return False
    ...
    def bst_to_list(bst):
        if bst == BinTree.empty:
            ...
            return [bst.label] + bst_to_list(bst.left) + bst_to_list(bst.right)
    tlist = bst_to_list(bst)
    return bst_min(bst) == min(tlist) and bst_max(bst) == max(tlist)
    """
    """
    def bst_validator(bst):
        if bst == BinTree.empty:
            pass
        elif bst.left < bst.label and bst.right > bst.label:
            return bst_validator(bst.left) + bst_validator(bst.right)
        else:
            return False
    """
# Q8
def add_up(n, lst):
    ...
    while index < len(lst):
        temp = n - lst[index]
        if temp in tset and temp != lst[index]:
            return True
        index += 1
    return False
