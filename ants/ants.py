    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            #        else:
                    ...
                    if insect.name == 'Queen' and not insect.impostor:
                        pass
                    elif insect.name == 'Queen' and insect.impostor:
                        self.ant = None
                    elif not insect.name == 'Queen':
                        self.ant = None
                    else:
                        pass
            else:
        ...
        if insect.name == 'Queen' and insect.impostor:
            insect.place = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # Make sure it's greater than the min distance
        if loc_index < self.min_range:
            while loc_index <= self.min_range and self.loc != hive:
                loc_index += 1
                self.loc = self.loc.entrance

        # While it's less than the max_range, go through possibilities
        while (self.loc != hive) and loc_index <= self.max_range+1:
            if len(self.loc.bees) != 0:
                return random_or_none(self.loc.bees)
            else:
                loc_index += 1
                self.loc = self.loc.entrance
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # Reduce armor by dmg taken
        self.armor -= amount

        # Make a list of all the bees
        blist = self.place.bees
        blist_copy = blist[:]

        index = 0

        # If RIP fireant, do dmg to all the bees and remove fireant from place
        if self.armor <= 0:
            while index < len(blist_copy):
                blist_copy[index].reduce_armor(self.damage)
                index += 1
            self.place.remove_insect(self)
        # END Problem 4A
...
class HungryAnt(Ant):
    ...
    def __init__(self, digesting_time = 3):
        # BEGIN Problem 6B
        Ant.__init__(self)
        self.time_to_digest = digesting_time
        self.digesting = 0
        # END Problem 6B
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            a = random_or_none(self.place.bees)
            self.eat_bee(a)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            self.throw_at(self.nearest_bee(colony.hive))
            tloc = self.place.exit
            while tloc != None:
                if tloc.ant != None:
                    if hasattr(tloc.ant, 'container'):
                        if tloc.ant.ant != None:
                            if not tloc.ant.ant in self.beenDoubled:
                                if not tloc.ant.ant.beenDoubledOG:
                                    tloc.ant.ant.damage *= 2
                                    tloc.ant.ant.beenDoubledOG = True
                                    self.beenDoubled.append(tloc.ant.ant)
                        if not tloc.ant in self.beenDoubled:
                            if not tloc.ant.beenDoubledOG:
                                tloc.ant.damage *= 2
                                tloc.ant.beenDoubledOG = True
                    else:
                        if not tloc.ant in self.beenDoubled:
                            tloc.ant.damage *= 2
                            self.beenDoubled.append(tloc.ant)
                #    else:
                #        if tloc.ant.ant != None:
                #            tloc.ant.ant.damage = tloc.ant.ant.originalDmg
                #            tloc.ant.damage = tloc.ant.originalDmg
                #        else:
                #            tloc.ant.damage = tloc.ant.originalDmg
                tloc = tloc.exit
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def new_action(durationRemaining):
        def new_action2(colony):
            nonlocal durationRemaining
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    setattr(bee, 'action', effect(bee.action)(duration))
    # END Problem EC
