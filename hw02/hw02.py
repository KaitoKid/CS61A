...
def filtered_accumulate(combiner, base, pred, n, term):
    ...
    """
    "*** YOUR CODE HERE ***"
    return accumulate(lambda x, y: combiner(x, y) if pred(y) else add(x, 0), base, n, term)
...
def pingpong(n):
    ...
    def is_multiple(i):
        if i == 0:
            return False
        return i % 7 == 0
    def index_finder(length, index, value, is_flipped):
        if length == 0:
            return value
        if has_seven(index-1) or is_multiple(index-1):
            return index_finder(length - 1, index + 1, value_mod(not is_flipped, value), not is_flipped)
        return index_finder(length - 1, index + 1, value_mod(is_flipped, value), is_flipped)
    return index_finder(n, 1, 0, False)
...
def count_change(amount):
    ...
    def method_finder(a, b):
        ...
        if a == 0:
            ...
            return method_finder(a - pow(2, b), b) + method_finder(a, b - 1)
...
def move_stack(n, start, end):
    ...
    temp = get_not_start_or_end(start, end)
